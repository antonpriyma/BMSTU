...
func(avt *Mealy) equal(avt1 Mealy)  {
	if (avt.n!=avt1.n || avt.m!=avt1.m || avt.q1!=avt1.q1) {
		fmt.Println("NOT EQUAL")
		return
	}
	for i:=0;i<avt.n;i++{
		for j:=0;j<avt.m;j++ {
			if (avt.delta[i][j]!=avt1.delta[i][j]) {
				fmt.Println("NOT EQUAL")
				return
			}

			if (avt.fun[i][j]!=avt1.fun[i][j]) {
				fmt.Println("NOT EQUAL")
				return
			}
		}
...
func(avt *Mealy) toMur(newi []int,newj []int)  ([]int,[]int){
	for i := 0; i < avt.n; i++ {
		for j := 0; j < avt.m; j++ {
			f := 0
			for z := 0; z < len(newi); z++ {
				if (avt.delta[newi[z]][newj[z]] == avt.delta[i][j]) && (avt.fun[newi[z]][newj[z]] == avt.fun[i][j]) {
					f = 1
					break
				}
			}
...
func main() {
	...
	var avt Mealy
	avt.in=make([]string,n)


	for i := 0; i < n; i++ {
		input.Scanf("%s", &avt.in[i])
	}

	input.Scanf("%d", &m)

	avt.out = make([]string, m)
	for i := 0; i < m; i++ {
		input.Scanf("%s", &avt.out[i])
	}

	input.Scanf("%d",&q1)
	avt.delta=make([][]int,q1)
	avt.fun=make([][]string,q1)

	for j:=0;j<q1;j++{
		avt.delta[j]=make([]int,n)
		avt.fun[j]=make([]string,n)
	}
	...
	newi := make([]int, 0, q1*n)
	newj := make([]int, 0, q1*n)

	newi,newj=avt.toMur(newi,newj)
	fmt.Printf("digraph {\nrankdir = LR\n")

	for i := 0; i < len(newi); i++ {
		fmt.Printf("%s%d [label = \"(%d,%s)\"]\n", avt.fun[newi[i]][newj[i]], avt.delta[newi[i]][newj[i]], avt.delta[newi[i]][newj[i]], avt.fun[newi[i]][newj[i]])
		for j := 0; j < avt.m; j++ {
			fmt.Printf("%s%d -> %s%d [label = \"%s\"]\n", avt.fun[newi[i]][newj[i]], avt.delta[newi[i]][newj[i]], avt.fun[avt.delta[newi[i]][newj[i]]][j], avt.delta[avt.delta[newi[i]][newj[i]]][j], avt.in[j])
		}
	}
