...
func main() {
	...
	var avt1 Mealy
	avt1.delta = make([][]int, n)
	avt1.fun = make([][]string, n)
	avt1.n = n
	avt1.m = m
	avt1.q1 = q1
	for j := 0; j < n; j++ {
		avt1.delta[j] = make([]int, m)
		avt1.fun[j] = make([]string, m)
	}
	...
	for i := 0; i < 2; i++ {
		for j := 0; j < n; j++ {
			for k := 0; k < m; k++ {
				switch i {
				case 0:
					input.Scanf("%d", &avt1.delta[j][k])
				case 1:
					input.Scanf("%s", &avt1.fun[j][k])
				}
			}
		}
	}

	avt = avt.AufenkampHohn()
	avt = avt.canon()
	avt1 = avt1.AufenkampHohn()
	avt1 = avt1.canon()
	avt.equal(avt1)
	//avt.printLikeDOT()
	//avt1.printLikeDOT()
