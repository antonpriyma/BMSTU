...
class GetConversionsListUsecase @Inject constructor(private val conversions: Repository,
  ...
  override fun buildUseCaseSingle(params: Params): Single<List<Conversion>> {
      var conv = conversions.getConversions(params.fromType, params.toTypes)
      return conv.map {
          it.map{ Conversion(date = Date(it.time).toInstant().atZone(ZoneId.systemDefault()).toLocalDate(),high = it.high, low = it.low,  open = it.open, close = it.close)}
      }
  }
  }
